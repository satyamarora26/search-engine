...
from app.services.crawl_runner import CrawlRunner
...
def crawl_medium_task(task: Task, job_id: str) -> dict[str, Any]:
    ...
    return execute_medium_crawl_attempt(
        task,
        UUID(job_id),
        runner_factory=CrawlRunner,
...
def execute_medium_crawl_attempt(
    ...
    *,
    ...
    try:
        ...
        logger.exception("Medium crawl job %s exhausted retries.", job_id)
        _record_final_failure(tracker, job_id)
        raise
    except Exception:
        logger.exception("Medium crawl job %s failed.", job_id)
        _record_final_failure(tracker, job_id)
        raise
...
def _record_final_failure(tracker: JobTracker, job_id: UUID) -> None:
    try:
        ...
        if job is None or job.job_type != MEDIUM_CRAWL_JOB:
            return
        tracker.mark_failure(job_id, error="Medium crawl failed.")
    except Exception:
        logger.exception("Could not record Medium crawl failure for %s.", job_id)
