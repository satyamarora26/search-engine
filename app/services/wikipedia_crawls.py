...
class WikipediaCrawlService:
    def __init__(
        ...
        *,
        ...
        self.crawls = crawl_repository or WikipediaCrawlRepository(session)
    ...
    def enqueue_crawl(self, request: WikipediaCrawlRequest) -> Job:
        ...
        try:
            ...
            self.crawls.create_run(
                job_id,
                root_category=request.category,
        ...
        *,
        ...
        try:
            ...
            if job is None or job.job_type != WIKIPEDIA_CRAWL_JOB:
                raise WikipediaCrawlNotFoundError(
                    f"Wikipedia crawl job {job_id} was not found."
                )
            total = self.crawls.count_item_views(job_id)
            items = self.crawls.list_item_views(
                job_id,
                limit=limit,
                offset=offset,
            )
            return total, items
        except WikipediaCrawlNotFoundError:
            raise
        except SQLAlchemyError as error:
            self.session.rollback()
            raise JobStorageError("Job storage unavailable.") from error
