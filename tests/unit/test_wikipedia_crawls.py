...
from app.schemas.wikipedia_crawls import WikipediaCrawlRequest
...
class FakeCrawlRepository:
    def __init__(self) -> None:
        ...
        self.frontier_created_with = None
        self.total = 0
        self.listed = []
        self.listed_with = None

    def create_run(self, job_id: UUID, **values):
        self.run_created_with = {"job_id": job_id, **values}
        if self.create_error is not None:
            raise self.create_error
        return SimpleNamespace(job_id=job_id, **values)

    def add_frontier(self, job_id: UUID, **values):
        self.frontier_created_with = {"job_id": job_id, **values}
        return SimpleNamespace(job_id=job_id, **values)
    ...
    def list_item_views(self, job_id: UUID, *, limit: int, offset: int):
        self.listed_with = {
            "job_id": job_id,
            "limit": limit,
            "offset": offset,
...
def test_unique_resource_race_reports_winning_job():
    service, session, jobs, crawls, task = service_fixture()
    winner = active_job(job_type=WIKIPEDIA_CRAWL_JOB)
...
def test_database_failure_rolls_back_and_maps_safe_storage_error():
    service, session, _, crawls, task = service_fixture()
...
def test_broker_failure_marks_pending_job_failed_with_safe_message():
...
def test_failure_recording_error_does_not_replace_enqueue_error():
    service, session, jobs, _, task = service_fixture()
...
def test_list_items_returns_total_and_stable_repository_page():
    service, _, jobs, crawls, _ = service_fixture()
    jobs.job = SimpleNamespace(id=JOB_ID, job_type=WIKIPEDIA_CRAWL_JOB)
    crawls.total = 3
    crawls.listed = [SimpleNamespace(position=1), SimpleNamespace(position=2)]

    total, listed = service.list_items(JOB_ID, limit=2, offset=1)

    assert total == 3
    assert [item.position for item in listed] == [1, 2]
    assert crawls.listed_with == {
        "job_id": JOB_ID,
        "limit": 2,
        "offset": 1,
    }
...
def test_list_items_rejects_unknown_or_non_crawl_job(job):
    service, _, jobs, _, _ = service_fixture()
    jobs.job = job

    with pytest.raises(
        WikipediaCrawlNotFoundError,
        match=f"Wikipedia crawl job {JOB_ID} was not found",
    ):
        service.list_items(JOB_ID, limit=100, offset=0)


def test_list_items_maps_database_failure_to_storage_error():
    service, session, jobs, crawls, _ = service_fixture()
    jobs.job = SimpleNamespace(id=JOB_ID, job_type=WIKIPEDIA_CRAWL_JOB)
    crawls.count_error = SQLAlchemyError("database hostname leaked")
