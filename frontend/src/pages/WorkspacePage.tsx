...
import type { SearchRanking, SearchResponse } from '../api/types'
...
export function WorkspacePage() {
  ...
  async function runSearch(nextQuery: string, nextRanking: SearchRanking) {
    setQuery(nextQuery)
    ...
    setError(null)
    try {
      setResponse(await searchDocuments(nextQuery, nextRanking))
    } catch (requestError) {
      setResponse(null)
      setError(
      ...
      <div className="workspace-grid">
        <div className="workspace-primary">
          <Panel className="search-panel" eyebrow="Search the index" title="What are you looking for?">
            <SearchForm initialQuery={query} initialRanking={ranking} isLoading={isLoading} onSubmit={runSearch} />
            {error && (
              <div className="inline-error search-error" role="alert">
                ...
                <button className="button button-quiet" type="button" onClick={() => void runSearch(query, ranking)}>Retry search</button>
              </div>
            )}
            <SearchResults hasSubmitted={hasSubmitted} isLoading={isLoading} response={response} />
          </Panel>
        </div>
